...
1. 这条规则主要匹配了哪些字段？
prot: tcp：匹配 TCP 协议的数据包
dport: 38060：匹配目标端口为 38060 的数据包
源地址 / 目标地址：默认 0.0.0.0/0，表示匹配所有地址的流量


2. 为什么服务端没有收到请求，也能说明防火墙已经在更前面拦截了流量？
网络数据包的流向是：客户端 → 内核网络栈（iptables 处理） → 服务端程序。当 iptables 在 INPUT 链中匹配到数据包并执行 REJECT 动作时，会直接在内核层面拒绝连接，数据包根本不会到达用户态的服务端程序，所以服务端不会收到任何请求日志，这就说明流量被防火墙提前拦截了。


3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？
两者在客户端都会表现为 Connection refused（连接被拒绝），用户无法区分是服务端没启动，还是防火墙拒绝了请求。区别在于：
服务端没启动：是操作系统内核返回的拒绝响应（因为没有进程监听该端口）
防火墙 REJECT：是 iptables 主动返回的拒绝响应（带 icmp-port-unreachable 标记）
...
| 你判断命中的依据 |1. 客户端请求成功，没有被拒绝；2. ACCEPT 规则的 pkts 计数器从 0 增加到 1；3. REJECT 规则的计数器没有新增请求 |

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？
因为使用 -I INPUT 1 命令把 ACCEPT 规则插入到了第 1 行，而 REJECT 规则在第 2 行。数据包匹配时会按顺序从上到下检查，客户端的请求先命中了第 1 行的 ACCEPT 规则，直接被放行，根本不会走到第 2 行的 REJECT 规则，所以访问恢复正常。


2. 为什么防火墙规则顺序会影响最终结果？
iptables 是顺序匹配、匹配即停止的机制：数据包会按规则列表从上到下依次匹配，一旦命中某条规则，就会执行对应的动作（ACCEPT/REJECT/DROP），并停止后续规则的检查。所以如果 ACCEPT 规则在前面，请求会被放行；如果 REJECT 规则在前面，请求会被拒绝，即使后面有放行规则也不会生效。


3. 如果把 `REJECT` 放在 `ACCEPT` 前面，本次访问会发生什么？
客户端的请求会先命中 REJECT 规则，被直接拒绝，访问失败，和之前的现象一致。即使后面有 ACCEPT 规则，数据包也不会再继续匹配，无法被放行。
...
| 使用 `REJECT` 时客户端失败现象 |立即失败，报错 Connection refused |
| 使用 `DROP` 时客户端失败现象 |超时失败，报错 timeout after 5.002s |
| `DROP` 是否比 `REJECT` 等待更久 |是，需要等待 5 秒超时才失败 |
| 服务端是否收到请求 |否 |
| `DROP` 规则计数器是否增加 |是，pkts 从 0 增加到 5 |

简答题：

1. `REJECT` 和 `DROP` 都能阻断访问，为什么客户端看到的现象不同？
REJECT：内核收到数据包后，会直接返回一个 ICMP 端口不可达 的响应，客户端立刻收到拒绝信号，所以表现为立即失败。
DROP：内核收到数据包后，直接丢弃且不返回任何响应，客户端一直收不到回应，只能等到超时时间结束，所以表现为超时失败。


2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？
REJECT 更容易判断问题。因为 REJECT 会明确返回拒绝响应，客户端能直接知道 “连接被拒绝”，说明请求已经到达了防火墙并被拦截，管理员可以快速定位到防火墙规则的问题；而 DROP 只表现为超时，无法区分是防火墙拦截、网络不通，还是服务端没启动，排错难度更高。



3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？
端口扫描工具通常会根据响应判断端口状态：
收到 Connection refused 会判定为 “端口关闭”
收到超时会无法区分是 “端口被防火墙丢弃” 还是 “网络不通 / 主机不存在”
所以 DROP 会让攻击者无法准确判断端口状态，增加了扫描结果的不确定性，也提升了防火墙的隐蔽性。
...
主要匹配传输层协议、目标端口、源 IP 地址。通过指定 TCP 协议、特定服务端口、本地回环 IP，对数据包首部信息进行匹配过滤。


2. 为什么说本实验实现的是“包过滤”，而不是“应用层代理”？
iptables 只依据IP 首部、TCP 首部的地址和端口做判断，不解析 HTTP 应用层内容。只看包头部，不看应用数据，所以属于网络层 / 传输层包过滤，不是应用层代理。



3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？
INPUT 是发给本机的入站数据包；OUTPUT 是从本机发出的出站数据包；FORWARD 是经过本机转发、不是发给本机也不是本机发出的中转数据包。


4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？
实验的客户端和服务端都在同一台主机访问本机服务，流量目的地址就是本机，只经过 INPUT 链。FORWARD 只用于跨主机路由转发，本实验没有转发行为，所以不用。


5. `ACCEPT`、`REJECT`、`DROP` 三种动作的区别是什么？
ACCEPT 直接放行数据包，程序可以正常接收请求；REJECT 拒绝数据包，同时返回端口不可达响应，客户端立刻提示连接被拒绝；DROP 直接静默丢弃数据包，不回任何响应，客户端只能一直等待直到超时失败。


6. 为什么规则顺序会影响最终结果？
iptables 是从上往下依次匹配，匹配到就执行动作并停止继续匹配。靠前的规则优先生效，就算后面有相反规则也不会再判断，所以顺序不同最终放行或拦截结果就会不一样。


7. 规则计数器在排错时有什么用？
计数器记录每条规则匹配到的数据包数量。可以直观判断流量有没有命中这条规则，方便排查规则是否生效、是否被前面规则优先匹配、以及流量是否被正常拦截或放行。


8. 真实环境中为什么常常采用“默认拒绝，再按需放行”的策略？
遵循最小安全权限原则，默认封闭所有端口和流量，只手动开放业务必需的端口和地址。能最大限度缩小攻击面，防止因漏配置规则产生安全漏洞，比默认放行更安全。


9. 只靠本实验这种基础包过滤规则，还无法解决哪些更复杂的安全问题？
无法识别和防范应用层攻击，如 SQL 注入、网页恶意脚本；无法检测 TCP 连接状态、防范会话劫持；无法抵御端口扫描、IP 地址伪造；也不能识别应用层协议违规流量，只能做简单的地址端口过滤。
