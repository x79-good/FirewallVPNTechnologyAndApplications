...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：方法、URL、协议、版本号、请求头、空行、请求体。

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404 表示服务器找不到请求的资源。500 是服务器内部出现未知错误，503 是服务器暂时无法处理请求，通常是过载或维护，属于临时故障。

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：GET 把数据放在 URL 中，长度有限制，可缓存，适合查询、获取数据。POST 把数据放在请求体中，长度无明显限制，相对更安全，适合提交表单、上传文件、发送敏感数据。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：HTTP 是明文传输，端口 80，不安全。HTTPS 是加密传输，端口 443，更安全，需要证书。HTTPS使用 SSL/TLS 协议，通过加密、身份验证、数据完整性校验来保护数据。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：HTTPS 是对网络传输过程加密，浏览器是通信的一端，会先解密数据再展示，所以开发者工具能看到明文。
