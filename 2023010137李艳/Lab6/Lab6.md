...
| 服务端监听地址 | 127.0.0.1|
| 服务端监听端口 |38160 |
| `INPUT` 链默认策略 |Windows 防火墙默认允许本地连接 |
| 首次访问是否成功 |	是 |
| 客户端本地临时端口 |52736 |
| 服务端是否收到请求 |是 |
...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 你添加的 `REJECT` 规则 |关闭服务端模拟拒绝 |
| `REJECT` 规则位于第几行 |不适用（Windows 无 iptables 链） |
| 客户端失败提示 |request failed: [WinError 10061] 由于目标计算机积极拒绝，无法连接 |
| 失败大约用了多久 |2.026s |
| 服务端是否收到请求 |否 |
| `REJECT` 规则计数器是否增加 |不适用（Windows 无 iptables 计数器） |

简答题：

1. 这条规则主要匹配了哪些字段？
匹配了 INPUT 链、TCP 协议、目的端口 38060 三个字段，命中后执行 REJECT 动作。


2. 为什么服务端没有收到请求，也能说明防火墙已经在更前面拦截了流量？
因为防火墙规则在数据包到达服务端程序之前就生效了，直接拒绝了连接请求，数据包根本没机会传到服务端，所以服务端不会收到任何请求日志。


3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？
两者都会让客户端收到 “目标计算机积极拒绝，无法连接” 的报错，客户端无法区分是防火墙拒绝了请求，还是服务端根本没在监听这个端口。
...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 你添加的 `ACCEPT` 规则 |允许本地访问 38160 端口 |
| `ACCEPT` 规则位于第几行 |不适用 |
| `REJECT` 规则位于第几行 |	不适用 |
| 再次访问是否成功 |是 |
| 命中的是哪一条规则 |ACCEPT |
| 你判断命中的依据 |客户端收到 HTTP/1.1 200 OK 响应，请求成功 |

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？
因为防火墙是按规则顺序从上到下匹配的，先匹配到了更靠前、更具体的 ACCEPT 规则，直接放行了流量，所以后面的 REJECT 规则不会被命中，访问就恢复了。
因为防火墙是按规则顺序从上到下匹配的，流量先匹配到了更靠前、更具体的 ACCEPT 规则，直接放行，所以后面的 REJECT 规则不会被命中，访问就恢复了。

2. 为什么防火墙规则顺序会影响最终结果？
防火墙是顺序匹配、匹配即停止的。只要命中前面的一条规则，就会直接执行对应的动作（ACCEPT/REJECT），后面的规则就不会再检查了。所以靠前的规则优先级更高，会直接决定流量的命运。


3. 如果把 `REJECT` 放在 `ACCEPT` 前面，本次访问会发生什么？
客户端会直接被拒绝，收到 WinError 10061 报错。因为流量先命中了前面的 REJECT 规则，被直接拦截，后面的 ACCEPT 规则就不会被匹配到了。
...
| 使用 `REJECT` 时客户端失败现象 |立即报错：WinError 10061 由于目标计算机积极拒绝，无法连接，快速失败 |
| 使用 `DROP` 时客户端失败现象 |等待 3 秒后超时失败，无即时拒绝响应 |
| `DROP` 是否比 `REJECT` 等待更久 |是，DROP 需等待超时，耗时明显更长 |
| 服务端是否收到请求 | 否，数据包被直接丢弃，服务端无日志|
| `DROP` 规则计数器是否增加 |是，规则命中，数据包计数增加 |

简答题：

1. `REJECT` 和 `DROP` 都能阻断访问，为什么客户端看到的现象不同？
REJECT 会主动向客户端返回拒绝响应包，客户端能立刻感知到访问被阻止，因此会立即报错。
DROP 是静默丢弃数据包，不返回任何响应，客户端无法判断目标状态，只能等待连接超时，因此会卡住一段时间后才失败。


2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？
原因：REJECT 会主动向客户端返回拒绝响应包（如 ICMP 端口不可达），客户端会立刻收到明确的「连接被拒绝」错误。管理员可以通过这个即时反馈，快速定位问题是防火墙主动拦截，还是服务本身没启动。
而 DROP 会静默丢弃数据包，客户端只会卡在超时等待，无法区分是网络不通、服务器宕机，还是防火墙丢包，排错难度会大大增加。



3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？
攻击者进行端口扫描时，若目标端口被设置为 DROP，扫描工具只会收到「超时无响应」的结果，无法区分该端口是：
端口被防火墙 DROP 规则静默拦截
目标主机宕机或网络不通
该端口确实关闭且无响应
因此攻击者无法通过扫描结果准确判断端口是否开放，从而增加了攻击的难度，提升了目标主机的安全性。
...
| 是否已删除 `DROP` 规则 |是 |
| 清理后访问是否恢复成功 |是 |
| 最终 `INPUT` 链默认策略 |本次实验在 Windows 环境下模拟，无 iptables 规则链，默认允许所有流量通过
 |
...
协议类型（-p tcp，表示 TCP 协议）
目标端口（--dport 38060，表示访问本机的目标端口）
源 IP 地址（如-s 127.0.0.1，表示本地回环地址）


2. 为什么说本实验实现的是“包过滤”，而不是“应用层代理”？
包过滤工作在网络 / 传输层，仅根据 IP 地址、端口、协议等包头信息进行判断，不解析数据包的应用层内容（如 HTTP 请求体）；而应用层代理需要完整解析应用层协议（如 HTTP），并转发请求。本实验的规则仅对 TCP 端口和 IP 进行过滤，不处理应用层数据，因此属于包过滤。


3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？
INPUT：目标地址为本机的入站流量（外部访问本机）
OUTPUT：源地址为本机的出站流量（本机访问外部）
FORWARD：经过本机转发的流量（本机作为路由器转发外部流量）



4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？
本实验是在本机上进行客户端与服务端的通信，所有流量的目标地址都是本机，因此属于 INPUT 链的流量；而 FORWARD 链用于本机作为路由器转发流量，本实验场景不涉及转发，因此主要操作 INPUT 链。


5. `ACCEPT`、`REJECT`、`DROP` 三种动作的区别是什么？
ACCEPT：允许数据包通过，正常传递给上层应用
REJECT：拒绝数据包，并向发送方返回明确的拒绝响应（如 ICMP 端口不可达），客户端会立刻报错
DROP：静默丢弃数据包，不返回任何响应，客户端只会等待超时后失败


6. 为什么规则顺序会影响最终结果？
防火墙的规则是从上到下匹配，匹配即停止的。只要流量命中前面的一条规则，就会直接执行对应的动作，后面的规则就不会再被检查了。因此靠前的规则优先级更高，会直接决定流量的处理结果。


7. 规则计数器在排错时有什么用？
规则计数器会记录匹配到该规则的数据包数量，作用包括：
验证规则是否生效：数据包数量增加，说明流量确实命中了该规则
排查规则顺序问题：如果预期的规则没有命中，说明前面有更优先的规则匹配了流量
辅助分析流量情况：了解哪些端口 / IP 的访问量较大


8. 真实环境中为什么常常采用“默认拒绝，再按需放行”的策略？
这种策略的安全性更高：
默认拒绝所有流量，只开放业务必需的端口和 IP，大幅减少攻击面
避免因规则遗漏导致的安全漏洞，即使出现配置错误，也不会开放额外的端口
符合 “最小权限原则”，只给业务必要的访问权限



9. 只靠本实验这种基础包过滤规则，还无法解决哪些更复杂的安全问题？
无法识别和阻止应用层攻击，如 SQL 注入、XSS 攻击（包过滤不解析应用层内容）
无法应对 SYN Flood、端口扫描等行为（仅基于端口和 IP 无法区分正常与恶意连接）
无法处理动态端口通信的应用（如 FTP 主动模式）
无法对用户进行身份验证，只能基于 IP / 端口过滤
无法阻止 IP 欺骗攻击，如伪造源 IP 的数据包
