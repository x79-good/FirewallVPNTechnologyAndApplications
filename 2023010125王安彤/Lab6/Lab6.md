...
| 客户端失败提示 | 	request failed: Connection refused|
| 失败大约用了多久 |极短，约 0.001 秒 |
| 服务端是否收到请求 |否 |
| `REJECT` 规则计数器是否增加 |是 |
...
防火墙在网络层就拦截了数据包，请求没有到达应用层，所以服务端收不到。


3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？
都会提示 Connection refused，无法直接区分。
...
| 再次访问是否成功 | 成功|
| 命中的是哪一条规则 | ACCEPT 规则|
| 你判断命中的依据 | ACCEPT 计数器增加，访问成功|

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？
ACCEPT 排在 REJECT 前面，先匹配放行，不再执行后面的拒绝规则。


2. 为什么防火墙规则顺序会影响最终结果？
iptables 从上到下匹配，匹配即停止，顺序决定最终动作。


3. 如果把 `REJECT` 放在 `ACCEPT` 前面，本次访问会发生什么？
访问会被拒绝，提示 Connection refused。


![alt text](allowed.png)
...
| 使用 `REJECT` 时客户端失败现象 | 立即拒绝，Connection refused|
...
REJECT 会返回拒绝包，客户端立刻知道；DROP 直接丢包，客户端只能等超时。


2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？
REJECT 更容易，因为有明确拒绝信息，能快速定位是防火墙拦截。


3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？
无法区分端口关闭、主机离线还是被丢弃，扫描结果不准。



![alt text](drop_counter.png)
...
只检查 IP、端口、协议等包头信息，不解析应用层内容。


3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？
INPUT：访问本机；OUTPUT：本机发出；FORWARD：经过本机转发。



4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？
流量是访问本机服务，走 INPUT 链；没有跨主机转发，不需要 FORWARD。


5. `ACCEPT`、`REJECT`、`DROP` 三种动作的区别是什么？
ACCEPT：放行；REJECT：拒绝并返回错误；DROP：静默丢弃。


6. 为什么规则顺序会影响最终结果？
iptables 按顺序匹配，匹配到就执行，不再往下检查。


7. 规则计数器在排错时有什么用？
看 pkts/bytes 是否增加，判断流量是否命中某条规则。
...
无法防御应用层攻击、无法识别加密威胁、无法抗 DDoS、不支持状态检测。
