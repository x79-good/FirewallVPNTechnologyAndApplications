...
| 失败大约用了多久 |0.001s 左右（立刻失败）|
| 服务端是否收到请求 |否（服务端终端没有新的请求日志） |
| `REJECT` 规则计数器是否增加 | 是（pkts 从 0 变为 1）|
...
服务端没收到请求，说明防火墙在内核层面就把包拦截了，根本没发到用户态的服务进程


3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？
现象相似：两者都会让客户端收到 Connection refused，无法建立连接
...
| 再次访问是否成功 |是（request succeeded） |
| 命中的是哪一条规则 |	ACCEPT |
| 你判断命中的依据 |ACCEPT 的计数器 pkts 增加 |

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？
因为 ACCEPT 规则在 REJECT 前面，数据包先匹配到 ACCEPT 就直接放行，不会走到 REJECT。


2. 为什么防火墙规则顺序会影响最终结果？
因为 iptables 从上到下匹配，匹配到第一条就执行动作，不再往下走。
...
| 使用 `REJECT` 时客户端失败现象 |立刻返回 Connection refused |
| 使用 `DROP` 时客户端失败现象 |等待超时后失败|
| `DROP` 是否比 `REJECT` 等待更久 | 是（5 秒超时）|
| 服务端是否收到请求 |否|
| `DROP` 规则计数器是否增加 |是 |
...
REJECT会返回拒绝包，客户端立刻收到反馈；DROP 直接丢弃包不回应，客户端只能等超时。


2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？
REJECT更容易判断，因为能立刻知道是被拒绝了；DROP容易误以为网络不通。


3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？
DROP让攻击者无法判断端口是否开放，增加扫描难度。
...
| 是否已删除 `REJECT` 规则 | 是|
| 是否已删除 `DROP` 规则 |是 |
| 清理后访问是否恢复成功 |是（request succeeded） |
...
本实验中的包过滤规则主要匹配了：协议（TCP）、目的端口（38060）、源 IP 地址（127.0.0.1）。


2. 为什么说本实验实现的是“包过滤”，而不是“应用层代理”？
本实验仅根据 IP 层和传输层信息（IP、端口、协议）进行过滤，不解析 HTTP 应用层内容，因此属于 “包过滤” 而非 “应用层代理”。


3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？
INPUT 链处理目的地址为本机的流量；OUTPUT 链处理从本机发出的流量；FORWARD 链处理经过本机转发的流量。


4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？
本实验访问的是本机服务，流量属于 INPUT 链的处理范围，因此主要操作 INPUT 链。
...
6. 为什么规则顺序会影响最终结果？
iptables 是从上到下匹配规则，匹配到第一条规则后就执行动作，因此规则顺序直接影响最终结果。


7. 规则计数器在排错时有什么用？
规则计数器（pkts/bytes）可以直观地显示该规则是否被命中，帮助判断哪条规则生效。


8. 真实环境中为什么常常采用“默认拒绝，再按需放行”的策略？
“默认拒绝，按需放行” 的策略安全性更高，它默认阻断所有流量，只开放必要的服务，减少攻击面。


9. 只靠本实验这种基础包过滤规则，还无法解决哪些更复杂的安全问题？
基础包过滤无法防御应用层攻击（如 SQL 注入）、无法识别伪装 IP、无法处理动态端口协议，也无法实现基于用户的访问控制。
