...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |0xb550|
| 查询域名（Query Name）       |mobile.events.data.microsoft.com|
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用是将容易记忆的域名（如 www.yxnu.edu.cn）解析为计算机能识别的 IP 地址。
   访问网站必须先进行 DNS 解析，因为网络通信依靠 IP 地址寻址，而人类无法记住大量数字 IP，DNS 实现了域名到 IP 的转换。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：请求包与响应包的 Transaction ID 一致。
   该字段是 DNS 事务的唯一标识，用于匹配对应的请求和响应，防止收到错误或过期的 DNS 回复。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录表示将域名映射到对应的 IPv4 地址，是最常用、最基础的 DNS 记录类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：DNS 默认使用 UDP 协议，端口 53。
   当响应数据过大（超过 512 字节）、需要区域传输（AXFR），或要求可靠传输时，会改用 TCP 协议。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：1.不支持 IPv6，只能解析 IPv4 地址，无法适配现代网络。2.线程不安全，在多线程程序中使用容易出现数据混乱。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：说明该网站使用了服务器集群、负载均衡、CDN 分发或高可用架构，目的是提高访问速度和服务稳定性。
