...
| 服务端监听端口 | 38060|
| `INPUT` 链默认策略 | ACCEPT|
| 首次访问是否成功 |是（request succeeded） |
| 客户端本地临时端口 | 客户端输出local socket后的端口（随机）|
| 服务端是否收到请求 | 是|
...
匹配字段：协议（TCP）、目的端口（38060）、入站流量（INPUT 链）。

2. 为什么服务端没有收到请求，也能说明防火墙已经在更前面拦截了流量？

服务端未收到请求的原因：防火墙在网络层拦截数据包，数据包根本没有到达服务端应用程序。

3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？

REJECT 与服务端未启动的相似点：客户端都会快速提示连接失败，无等待延迟。
...
| 再次访问是否成功 | 是|
| 命中的是哪一条规则 | ACCEPT 规则|
| 你判断命中的依据 |ACCEPT 规则计数器增加，REJECT 无变化 |

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？

访问恢复原因：ACCEPT规则在REJECT前面，本机流量匹配到允许规则后直接放行，不再执行后续拒绝规则。

2. 为什么防火墙规则顺序会影响最终结果？

规则顺序影响结果：iptables 规则从上到下依次匹配，匹配到第一条规则后立即执行动作，不再匹配后续规则。

3. 如果把 `REJECT` 放在 `ACCEPT` 前面，本次访问会发生什么？

REJECT 在前的结果：流量会先匹配 REJECT 规则，访问直接被拒绝，无法成功。
...
| 使用 `REJECT` 时客户端失败现象 |立即拒绝，无等待 |
| 使用 `DROP` 时客户端失败现象 |超时等待后失败 |
...
现象不同原因：REJECT 会给客户端返回拒绝报文，客户端立即知晓；DROP 直接丢弃数据包，客户端只能等待超时。

2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？

排错更易的动作：REJECT，因为能快速明确判断是被防火墙拦截，而非网络故障。

3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？

DROP 对攻击者的影响：攻击者无法区分是主机不存在、端口未监听还是被防火墙拦截，隐蔽性更强。
...
本实验包过滤规则匹配的字段协议类型（TCP）、源 IP 地址（127.0.0.1）、目的端口（38060）、流量方向（INPUT 链）。

2. 为什么说本实验实现的是“包过滤”，而不是“应用层代理”？

包过滤工作在网络层 / 传输层，仅检查 IP、端口、协议；应用层代理会解析 HTTP 等应用层数据，本实验未处理任何应用层内容。

3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？

INPUT/OUTPUT/FORWARD 流量方向
INPUT：外部访问本机的入站流量
OUTPUT：本机访问外部的出站流量
FORWARD：经过本机转发的流量（网关 / 路由器场景）

4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？

实验是外部客户端访问本机服务，流量进入本机，属于 INPUT 链管控范围。

5. `ACCEPT`、`REJECT`、`DROP` 三种动作的区别是什么？

ACCEPT/REJECT/DROP 区别
ACCEPT：放行数据包
REJECT：拒绝并返回拒绝报文，客户端立即感知
DROP：静默丢弃数据包，无任何响应，客户端超时

6. 为什么规则顺序会影响最终结果？

iptables 按从上到下顺序匹配，匹配到第一条规则后立即执行动作，不再匹配后续规则。

7. 规则计数器在排错时有什么用？

查看pkts/bytes可以判断规则是否被命中，快速定位流量匹配了哪条防火墙规则。

8. 真实环境中为什么常常采用“默认拒绝，再按需放行”的策略？

最小权限原则，默认拦截所有流量，仅按需放行必要业务，避免遗漏规则导致安全风险。

9. 只靠本实验这种基础包过滤规则，还无法解决哪些更复杂的安全问题？

无法防护应用层攻击（如 SQL 注入、XSS）、无法识别恶意域名、无法管控用户行为、无法防御 DDoS 攻击。
