...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：请求行、请求头（请求头部）、空行、请求体

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404 表示请求的资源不存在；500 是服务器内部未知错误，503 是服务器暂时不可用，属于临时性故障。

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：区别：GET 数据拼在 URL 中、有长度限制、可缓存、幂等；POST 数据在请求体、无长度限制、不可缓存、非幂等。适用场景：GET 用于查询数据；POST 用于提交数据、上传文件等。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：区别：HTTP 明文传输、默认 80 端口、不安全；HTTPS 基于 SSL/TLS 加密、默认 443 端口、更安全。HTTPS 用对称加密 + 非对称加密 + 数字证书实现数据加密、身份认证和完整性校验。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：开发者工具展示的是浏览器本地解密后的数据，HTTPS 仅加密网络传输过程，不保护客户端本地的解析内容。
