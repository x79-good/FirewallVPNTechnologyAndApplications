...
| 项目                         | 你的填写内容 |
| :--------------------------- | :----------- |
| 本机 IP 地址                 |  172.19.146.78  |
| 本机所在子网                 |  172.19.0.0/16 |
| 子网掩码                     | 255.255.0.0 |
| 默认网关 IP                  | 172.19.0.1  |
| 默认网关 MAC 地址            |  c0-b8-e6-47-12-1e  |
| 本机网卡 MAC 地址            |  f4-c8-8a-f7-c2-09 |

简答题：

1. 路由表的每一行包含哪些关键字段？教材中提到的 `Network Destination`、`Netmask`、`Gateway`、`Interface` 分别对应什么含义？
答：
Network Destination：表示这条路由可以到达的目标网络或主机 IP。
Netmask：和目标 IP 一起，用来判断数据包是否属于这个网段。
Gateway：下一跳网关的 IP 地址（数据包要转发到的下一个节点）。
Interface：本机用来发送该数据包的网卡 IP 地址。

2. 当目标 IP 地址不在本子网时，包会先发给谁？路由表的哪一列提供了这个信息？

答：会先发给默认网关。
路由表中Network Destination为0.0.0.0的那一行，其Gateway列提供了这个信息。

1. 路由表的默认网关（`0.0.0.0`）条目的作用是什么？什么时候会匹配到这一行？

答：作用：这是默认路由，当没有其他更精确的路由条目匹配目标 IP 时，数据包会走这条路由，发给默认网关。
匹配时机：目标 IP 地址不属于任何直连子网，也没有其他静态路由条目匹配时。

4. 教材提到，确定发送方 IP 地址的关键在于"判断应该使用哪块网卡"。结合你查到的本机网卡信息，说明 IP 模块是如何做出这个判断的。

答：IP 模块会将目标 IP 地址，和本机所有网卡的 IP 地址 / 子网掩码进行匹配，看目标 IP 是否在该网卡的子网内。如果匹配到，就使用对应的网卡直接发送 ARP 请求。如果都不匹配，就使用默认路由对应的网卡，将数据包发给网关。
...
答：观察到的 UDP 头部长度是 8 字节。
UDP 省略了 TCP 头部中的序列号、确认号、窗口大小、紧急指针、选项等字段。
后果：
优点：头部开销极小，传输延迟低，适合对实时性要求高的场景。
缺点：不保证数据的可靠交付、不保证按序到达，也没有流量控制和拥塞控制，丢包、乱序都需要应用层自己处理

1. UDP 头部中的"长度"字段指的是什么长度？

答：它指的是整个 UDP 数据报的总长度，包括 8 字节的 UDP 头部 + 后面的应用层数据。
...
| 字段名称               | 你的填写内容 | 含义说明 |
| :--------------------- | :----------- | :------- |
| Version（版本号）      |     4     | 表示这是 IPv4 协议。 |
| Header Length（头部长度） | 20 bytes | IP 头的长度，单位是 4 字节，5对应 20 字节，说明这是一个标准 IP 头，没有选项字段。 |
| Time to Live（TTL）    |  128   |  生存时间，每经过一个路由器就减 1，防止数据包无限循环。 |
| Protocol（协议号）     |    17      |  表示 IP 层承载的上层协议是 ICMP |
| Source Address（源 IP） |  172.19.146.78   | 发送方的 IP 地址，即你的电脑。  |
| Destination Address（目的 IP） | 172.19.0.1 |  接收方的 IP 地址，即网关 |
...
答：协议号是1，代表 ICMP 协议。
HTTP 请求是基于 TCP 的，对应的协议号是6。

2. TTL 字段的作用是什么？如果 TTL 降为 0 会发生什么？

答：作用：限制数据包在网络中传输的跳数，防止因路由错误导致数据包无限循环。
如果 TTL 降为 0，数据包会被路由器丢弃，并发送一个 ICMP 超时错误（Time Exceeded）给源主机。
...
答：关键的几块：
无线网卡：172.19.146.78（当前上网的网卡）
VMware 虚拟网卡 1：192.168.119.1
VMware 虚拟网卡 8：192.168.150.1
回环接口：127.0.0.1

4. IP 头部中的源 IP 地址和目的 IP 地址分别是谁的地址？它们与 MAC 头部中的源/目的 MAC 地址有什么区别？

答：IP 地址：是三层逻辑地址，用于跨网络寻址，在传输过程中，源 / 目的 IP 始终不变。
MAC 地址：是二层物理地址，仅用于同一网段内寻址，每经过一个路由器，MAC 地址都会变成上一跳和下一跳的地址。
...
| 字段名称               | 你的填写内容 | 含义说明 |
| :--------------------- | :----------- | :------- |
| Source（源 MAC）       | f4:c8:8a:f7:c2:09 | 发送方的网卡物理地址  |
| Destination（目的 MAC） |c0:b8:e6:47:12:1e  |  接收方（网关）的物理地址 |
| Type（以太类型）       | 0x0800 | 表示帧内承载的是 IPv4 协议包 |
...
| :----------------- | :----------- |
| MAC 地址长度       | 48 比特（6 字节） |
| 本机网卡的 MAC 地址 | f4:c8:8a:f7:c2:09 |
| 目的 MAC 地址      | c0:b8:e6:47:12:1e  |
| MAC 地址的书写格式 | 十六进制，用冒号分隔（如xx:xx:xx:xx:xx:xx） |

简答题：

1. 以太类型字段的值是多少？它代表后面承载的是什么协议的包？

答：值为0x0800，代表后面承载的是IPv4 协议的包。

2. DNS 服务器的 IP 通常是外网地址。本任务中目的 MAC 地址是 DNS 服务器的 MAC 地址还是你本机网关（路由器）的 MAC 地址？为什么？
答：是本机网关（路由器）的 MAC 地址。
因为 DNS 服务器的 IP 是外网地址，不在你电脑的直连网段内，所以数据包必须先发给网关，由网关转发到外网，因此目的 MAC 是网关的。


3. IP 地址和 MAC 地址在功能上有什么相似之处？又有什么本质区别？

答：相似点：都是用来标识网络中的设备，作为数据包的源 / 目的地址。
本质区别：
IP 地址是三层逻辑地址，用于跨网络寻址，可配置、可变。
MAC 地址是二层物理地址，用于同一网段内寻址，烧录在网卡中，通常固定不变。


4. 为什么以太网帧中需要同时有 IP 地址（在 IP 头部中）和 MAC 地址？不能只用其中一种吗？

答：不能。IP 地址负责端到端的跨网寻址，在传输全程保持不变；MAC 地址负责链路层的跳对跳转发，每经过一个路由器就会更新。两者分工协作，缺一不可。
...
答：因为 ARP 请求需要让局域网内所有设备都收到，从而找到目标 IP 对应的 MAC 地址，所以用广播地址ff:ff:ff:ff:ff:ff发送

2. 为什么 ARP 缓存中的条目会在几分钟后自动删除？

答：为了防止 IP-MAC 映射过时（比如设备更换、IP 地址变化），自动删除并重新请求可以保证映射的准确性。

3. 如果 ARP 缓存中的 MAC 地址已经过期（对方 IP 对应的设备已更换），会出现什么问题？如何解决？

问题：无法与目标设备通信，因为数据包被发送到了错误的 MAC 地址。
解决：以管理员身份运行 CMD，执行arp -d *清除缓存，再重新 ping 目标 IP，触发新的 ARP 请求。
...
| 局域网设备（网关） |        是      |   平均 2ms    | 64 |
| 8.8.8.8            |      是        |   平均 194ms   |  108  |
...
答：发送的是Type 8（Echo Request），收到的回复是Type 0（Echo Reply）

2. 为什么 ping 不同目标的 TTL 值不同？TTL 值反映了什么信息？

答：TTL 是 IP 数据包的生存时间，每经过一个路由器就减 1，不同目标经过的路由器数量不同，所以 TTL 不同。
TTL 反映了数据包从源主机到目标主机经过的跳数，也能反映目标主机的操作系统（如 Windows 默认 TTL=128，Linux 默认 TTL=64）。
...
答：当目标 IP 不存在、目标端口未开放，或者路由表中没有到目标的路由时，路由器会返回 ICMP 类型 3（目的不可达）消息。
...
网络包由以太网帧（MAC 头部 + 数据）、IP 头部、传输层头部（TCP/UDP）和数据部分构成。
IP 头部：负责跨网寻址，提供源 / 目的 IP、TTL、协议号等信息。
MAC 头部：负责同一网段内的链路层寻址，提供源 / 目的 MAC 地址

2. IP 协议和以太网协议在网络传输中分别承担什么职责？它们是如何分工协作的？

IP 协议（三层）：负责逻辑寻址和路由选择，决定数据包的端到端路径。
以太网协议（二层）：负责物理链路的帧传输，决定数据包在当前网段内的下一跳。
协作方式：IP 地址全程不变，MAC 地址每跳更新，两者配合完成跨网络通信。

3. ARP 协议解决的核心问题是什么？如果不使用 ARP 缓存，网络中会出现什么情况？

核心问题：将 IP 地址解析为对应的 MAC 地址，实现三层地址到二层地址的映射。
没有 ARP 缓存：每次通信都要发送 ARP 广播请求，会增加网络广播流量，导致网络拥塞，通信效率大幅下降。

4. 为什么 IP 和负责传输的网络（如以太网）要分开设计？这种设计带来了什么好处？

为了实现网络层与链路层的解耦，让 IP 协议可以运行在不同的物理网络（以太网、Wi-Fi、PPP 等）上，提高了网络协议的灵活性和可扩展性

5. 网卡在发送包时会额外添加哪 3 个控制数据？它们各自的作用是什么？

前导码（Preamble）：用于帧同步，让接收方准备接收数据。
帧起始定界符（SFD）：标识帧的开始。
帧校验序列（FCS）：用于检测传输过程中是否发生错误。

6. 网卡接收到一个包后，需要经过哪些步骤才能将其交给操作系统？如果 FCS 校验失败会怎样？

步骤：接收帧→检查目的 MAC 是否匹配→FCS 校验→校验成功后去掉前导码和 FCS，将数据交给 IP 层。
FCS 校验失败：网卡会直接丢弃该帧，不会向上传递给操作系统。

7. TCP 和 UDP 的核心区别是什么？请从连接管理、可靠性、效率、适用场景四个维度进行比较。

连接管理：TCP 面向连接，需握手建立连接；UDP 无连接，直接发送数据。
可靠性：TCP 保证数据可靠、按序交付；UDP 不保证交付，数据可能丢失或乱序。
效率：TCP 开销大、效率低；UDP 开销小、延迟低。
适用场景：TCP 用于文件传输、网页浏览等；UDP 用于游戏、音视频通话等。

8. UDP 适用于哪些场景？请结合教材内容解释为什么这些场景适合使用 UDP 而非 TCP。

适用场景：实时音视频通话、在线游戏、DNS 查询、直播等。
原因：这些场景对实时性要求高，对少量数据丢失不敏感，UDP 的低延迟和高效率更能满足需求。

9. 如果一个 IP 包经过多次路由转发后 TTL 降为 0，路由器会如何处理？这与教材中提到的哪种 ICMP 消息有关？

路由器会丢弃该数据包，并向源主机发送 ICMP 类型 11（Time Exceeded，超时）消息。
