...
答：匹配了 协议（tcp） 和 目标端口（--dport 38060） 两个字段。

2. 为什么服务端没有收到请求，也能说明防火墙已经在更前面拦截了流量？

答：因为客户端的连接请求在到达服务端之前，就被防火墙拦截并拒绝了，请求根本没有发到服务端，所以服务端不会有日志。

3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？

答：两者在客户端都表现为 Connection refused，现象是一样的，客户端无法直接区分是防火墙拒绝还是服务端未监听。
...
| 再次访问是否成功 |是 |
| 命中的是哪一条规则 |ACCEPT 规则 |
| 你判断命中的依据 |客户端请求成功，且服务端收到请求并记录日志；同时 ACCEPT 规则的计数器会增加 |

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？

答：因为 ACCEPT 规则被插入到了第 1 行，iptables 按顺序匹配规则，请求被第 1 行的 ACCEPT 规则放行，不会匹配到后面的 REJECT 规则

2. 为什么防火墙规则顺序会影响最终结果？

答：iptables 是按从上到下的顺序匹配规则的，一旦匹配到一条规则就会执行对应的动作，不会继续往下匹配。

3. 如果把 `REJECT` 放在 `ACCEPT` 前面，本次访问会发生什么？

答：客户端会直接收到 Connection refused，请求被拒绝，不会被 ACCEPT 规则放行。
...
| 使用 `REJECT` 时客户端失败现象 |立刻提示 Connection refused |
| 使用 `DROP` 时客户端失败现象 |等待超时后提示 request failed: timed out |
| `DROP` 是否比 `REJECT` 等待更久 | 是，等待了约 5 秒才报错|
| 服务端是否收到请求 |否 |
| `DROP` 规则计数器是否增加 |是 |
...
REJECT 会主动返回拒绝包，客户端立刻收到反馈；DROP 会无声丢弃数据包，客户端收不到任何响应，只能等到超时才知道失败。

2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？

REJECT 更容易排错，因为它能明确告诉客户端 “连接被拒绝”，可以直接定位到是防火墙规则问题；而 DROP 只会让客户端超时，无法直接判断是网络不通还是防火墙丢弃了包。

3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？

攻击者的扫描工具无法区分 “端口被防火墙丢弃” 和 “主机不在线 / 网络不通”，扫描结果会变得模糊，无法准确判断目标端口的真实状态。
...
答：匹配了 协议（tcp）、目标端口（--dport） 和 源IP地址（-s） 三个字段。

2. 为什么说本实验实现的是“包过滤”，而不是“应用层代理”？

答：因为 iptables 工作在网络 / 传输层，只根据 IP 地址、端口、协议等包的头部信息进行过滤，不会解析 HTTP 应用层内容；而应用层代理会解析完整的应用层协议，根据请求内容进行控制。

3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？

INPUT：目标地址是本机的入站流量。
OUTPUT：从本机发出的出站流量。
FORWARD：经过本机转发的流量（本机不是源也不是目标）。

4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？

答：因为实验中的客户端和服务端都在同一台主机上，流量都是本机的入站流量，所以只需要操作 INPUT 链即可；FORWARD 链用于跨主机的转发流量，本实验不需要

5. `ACCEPT`、`REJECT`、`DROP` 三种动作的区别是什么？

ACCEPT：允许数据包通过，继续后续处理。
REJECT：拒绝数据包，并主动向发送方返回拒绝响应。
DROP：丢弃数据包，不向发送方返回任何响应。

6. 为什么规则顺序会影响最终结果？

答：iptables 是按从上到下的顺序匹配规则的，一旦匹配到一条规则就会执行对应的动作，不会继续往下匹配，所以顺序会直接影响流量的最终走向。

7. 规则计数器在排错时有什么用？

答：计数器（pkts/bytes）可以直观地显示规则是否被命中。如果添加了规则但计数器一直为 0，说明规则没有匹配到任何流量，方便排查规则是否写错或顺序是否错误。

8. 真实环境中为什么常常采用“默认拒绝，再按需放行”的策略？

答：这种策略只开放业务必需的端口和服务，其他所有流量默认拒绝，可以最大程度减少攻击面，降低被攻击的风险，符合 “最小权限原则”。

9. 只靠本实验这种基础包过滤规则，还无法解决哪些更复杂的安全问题？

答：无法防御应用层攻击（如 SQL 注入、XSS）、端口扫描绕过、状态检测攻击，也无法根据用户、请求内容、时间等动态条件进行控制，只能实现简单的 IP / 端口过滤。
