...
| 字段                         | 你的截图中的值 |
| :--------------------------- | :------------- |
| 事务 ID（Transaction ID）    |    0Xdf36            |
| 查询域名（Query Name）       |   www.bing.com             |
| 查询类型（Type）             |     A(1)           |
| 目的端口                     |       53         |
...
| 回答记录数（Answers）        |      4          |
| 返回的 IP 地址               |  202.89.233.100 和 202.89.233.101     |
| TTL（若可见）                |     不可见           |
...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS（Domain Name System，域名系统）的主要作用是将人类易记的域名（如 www.bing.com）转换为机器可识别的 IP 地址（如 202.89.233.100）。访问网站时通常要先进行 DNS 解析，是因为网络通信（如 TCP/IP 协议栈）本质上是基于 IP 地址进行寻址和路由的，无法直接识别域名。只有获得目标服务器的 IP 地址后，才能建立连接并发送请求。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致。在提供的抓包中，请求和响应的 Transaction ID 都是 0xdf36。作用：Transaction ID（事务 ID）用于匹配 DNS 请求和对应的响应。由于 DNS 使用无连接的 UDP 协议，客户端发送请求后，需要靠这个 ID 来识别哪个响应属于哪个请求，防止乱序或错误匹配。同时它也能在一定程度上增强安全性（防止简单的欺骗攻击）。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 记录（Address Record）用于将域名解析为 IPv4 地址。例如 china.bing123.com → 202.89.233.100 就是一条 A 记录。与之对应，AAAA 记录用于解析为 IPv6 地址。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认协议和端口：UDP 53。改用 TCP 的常见情况：响应报文长度超过 512 字节（如含有大量记录或 DNSSEC 扩展时）。区域传输（Zone Transfer，主从 DNS 服务器之间同步数据）。某些安全或代理场景要求使用 TCP 保证可靠性。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：不支持 IPv6：gethostbyname() 只能返回 IPv4 地址（struct hostent 设计局限），无法处理 AAAA 记录。非线程安全：函数内部使用静态数据区，在多线程环境下会产生数据竞争（虽然有 gethostbyname_r，但接口复杂且不跨平台）。阻塞且不可取消：在解析缓慢或失败时会长时间阻塞，无法设置超时或被外部中断。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：通常说明该域名背后部署了 多台服务器，用于实现：负载均衡：将请求分散到不同的物理或虚拟服务器上，分担压力。高可用性/故障转移：当某台服务器不可用时，客户端可以尝试另一个 IP。地理分布/就近访问：不同 IP 可能对应不同地域的服务器（如 CDN），客户端可选择最快的一个。
