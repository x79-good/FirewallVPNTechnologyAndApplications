...
| 服务端监听地址                      |0.0.0.0              |
| 服务端监听端口                      | 38090             |
| 客户端本地临时端口                  |客户端打印的 local socket 端口（运行时自动显示）             |
| 客户端请求总字节数                  |200000              |
| 服务端响应内容                      |Hello TCP Lab4! This is server response.              |
| 客户端 `connect()` 返回前后的时间点 |脚本自动打印时间              |
| 客户端首次收到响应前等待了多久      |脚本打印：first recv() returned after X.XXXs              |
...
答：服务端在客户端连接前处于 LISTEN，是因为它调用了 listen()，表示准备接收客户端连接请求。


2. 为什么这时还没有真正建立 TCP 连接？
答：还没有完成三次握手，没有交换 SYN/ACK，所以没有真正建立 TCP 连接。


3. `socket()` 与 `connect()` 的区别是什么？
答：socket() 只是创建一个套接字对象；connect() 是主动发起三次握手、建立连接。


4. 为什么 `connect()` 返回后才进入可稳定收发数据的状态？
答：connect () 返回代表三次握手完成，双方进入 ESTABLISHED，可以稳定收发数据。


5. 为什么"网线一直连着"不等于"TCP 连接已经建立"？
答：网线连通只是物理层连通，TCP 连接需要传输层控制报文协商才能建立。


6. 这里的"连接"更准确地说是在做什么？
答：“连接” 本质是交换控制信息，初始化序号、窗口、MSS 等双方状态。
...
| :--------- | :----- | :------- | :--- | :--- | :---- | :----- | :------------ |
| 第一次握手 |客户端临时端口	        |  38090        |  0    |   0   |  SYN     |  65535      |     32          |
| 第二次握手 | 38090       |    客户端临时端口	      |    0	  |    1  |   SYN,ACK	    |    65535    |      32         |
| 第三次握手 |客户端临时端口	        |   38090       |  1	    |   1	   |  ACK     |  65535      |     20          |
...
端口号用来定位主机上的具体套接字 / 进程，实现多路复用。


2. TCP 头部如何帮助找到目标套接字？
通过 源 IP、目的 IP、源端口、目的端口 唯一确定一个套接字。


3. 为什么初始序号不是简单固定从 1 开始？
随机初始序号防止历史报文干扰、提高安全性。


4. 为什么 TCP 可选字段更容易在连接阶段看到？
可选字段（MSS / 窗口缩放 / SACK）只在握手阶段协商，数据报文不再携带。
...
| 项目                   | 你的填写内容 |
| :--------------------- | :----------- |
| 纯控制报文的类型       |SYN、ACK、FIN              |
| 携带应用数据的报文类型 | PSH,ACK             |
| 头部中的控制信息举例   |Flags、Seq、Ack、Window              |
| 套接字中的控制信息举例 |LISTEN/ESTABLISHED 状态、接收缓冲区、五元组             |

简答题：

1. 为什么"头部中的控制信息"和"套接字中的控制信息"不是同一件事？
头部控制信息在网络包上；套接字控制信息在操作系统内核里，不是一回事。
...
| ACK 号大致确认到了第几个字节 | 累计确认到已接收的最大字节         |

简答题：

1. 应用程序是否直接决定每个网络包的数据长度？为什么？
应用不直接决定包长，TCP 按 MSS 自动分段。


2. 大块应用数据为什么会被拆分？
超过 MSS 必须拆分，避免 IP 分片。
...
一次 sendall () 会被拆成多个 TCP 包。


5. 为什么 ACK 体现的是累计确认？
ACK 是累计确认，表示之前所有字节都收到。


6. 如果中间某一段丢失，ACK 会出现什么变化？
中间段丢失，ACK 会停在丢失位置重复确认，触发重传。
...
滑动窗口允许连续发送，同时限制发送量不超过接收能力，兼顾效率与安全。
...
| 客户端确认报文的 Ack     |  服务端 Seq + 数据长度            |

简答题：

1. 为什么 TCP 的 `seq/ack` 是双向分别计算的？
TCP 是全双工，两个方向是独立数据流，各自编号。
...
2. 为什么连接结束后套接字不会立刻删除？
TIME-WAIT 等待最后 ACK 可靠到达，防止旧报文干扰新连接。



3. 如果最后一个 ACK 丢失，而旧套接字已经立刻删除，可能带来什么问题？
被动关闭方会重传 FIN，导致连接异常、无法正常关闭。
...
TCP 连接是双方状态协商，不是物理连线；通过控制报文达成收发约定。



2. 三次握手为什么能让双方进入可通信状态？
三次握手让双方确认序号、窗口、MSS，进入可收发状态。



3. TCP 头部中的控制字段如何支撑收发数据？
头部字段（端口、Seq、Ack、Flags、Window）支撑寻址、可靠、流控。



4. ACK、窗口、等待时间为什么会共同影响 TCP 的可靠传输？
ACK 保证可靠，窗口限制发送速度，等待时间匹配 RTT，共同实现可靠传输。



5. 断开连接为什么仍然需要严格的控制信息交换？
断开必须交换 FIN/ACK，确保数据无丢失、双方都同意关闭。



6. 如果服务端根本没有启动，客户端调用 `connect()` 时会看到什么现象？
服务端未启动，connect () 会报错：连接拒绝（Connection refused）。



7. 如果中途人为制造丢包，ACK、重传、窗口之间会出现什么变化？
丢包会出现重复 ACK、快速重传、窗口临时下降。



8. 如果把客户端发送的数据改得更大，窗口字段和分段情况会如何变化？
数据更大 → 分段更多，窗口随接收能力动态调整。



9. 如果把服务端读取速度改得更慢，是否更容易看到窗口更新甚至零窗口？
服务端读得慢 → 窗口变小，更容易出现窗口更新甚至零窗口。
