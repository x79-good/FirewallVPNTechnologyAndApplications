...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS 的作用是将人类易于记忆的域名（如 www.baidu.com）转换为计算机能够识别的 IP 地址（如 110.242.68.3）。访问网站需要先进行解析，是因为网络层协议（IP 协议）依靠 IP 地址进行路由和寻址，计算机无法直接根据字符形式的域名找到目标服务器。

2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答：一致。Transaction ID（事务 ID）用于标识一个特定的查询会话。由于 DNS 通常使用无连接的 UDP 协议，客户端通过匹配响应包与请求包中相同的 ID，来确保收到的回复确实是针对自己发出的那个请求。

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A 代表 Address。A 记录用于将域名映射到一个 IPv4 地址。它是 DNS 中最基础、最常用的记录类型。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用 UDP 协议，端口号为 53。当 DNS 响应报文的数据长度超过 512 字节（例如包含大量 A 记录或使用 DNSSEC 安全扩展时），或者在进行 区域传输（Zone Transfer） 时，会改用 TCP 协议 以保证传输的可靠性。

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：1. 不支持 IPv6：该接口仅设计用于处理 IPv4 地址，无法满足现代网络 IPv6 化的需求。2. 非线程安全：它返回一个指向静态分配内存的指针，在多线程环境下同时调用会导致数据竞争和结果错误。

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：这通常说明该网站使用了 负载均衡（Load Balancing） 或 冗余备份 技术。通过将流量分配到不同的服务器 IP，可以提高网站的并发处理能力和可用性，防止单点故障。
