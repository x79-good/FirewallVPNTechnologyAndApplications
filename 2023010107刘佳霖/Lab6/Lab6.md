...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 服务端监听地址 |	127.0.0.1:38060 |
| 服务端监听端口 |38060 |
| `INPUT` 链默认策略 |ACCEPT |
| 首次访问是否成功 |是 |
| 客户端本地临时端口 |127.0.0.1:52456 |
| 服务端是否收到请求 |是 |
...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 你添加的 `REJECT` 规则 |netsh advfirewall firewall add rule name="Block_Lab6_38060" dir=in action=block protocol=tcp localport=38060 |
| `REJECT` 规则位于第几行 |第 2 行（在允许规则后面，生效顺序靠后） |
| 客户端失败提示 |立刻失败，提示 Connection refused |
| 失败大约用了多久 |约 0.005 秒（几乎立刻） |
| 服务端是否收到请求 |否 |
| `REJECT` 规则计数器是否增加 |已匹配数据包数量增加 |

简答题：

1. 这条规则主要匹配了哪些字段？
答：匹配了 TCP 协议、目的端口 38060、入站方向（INPUT），对所有访问本机 38060 端口的 TCP 连接执行拒绝动作。


2. 为什么服务端没有收到请求，也能说明防火墙已经在更前面拦截了流量？
答：客户端发送的连接请求在到达本机服务端之前，就被防火墙规则提前拦截并拒绝，因此服务端完全收不到任何连接日志，说明流量被防火墙在更早的规则链中拦截了。



3. `REJECT` 和“服务端程序没启动”在客户端现象上可能有什么相似之处？
答：两者都会导致客户端收到 Connection refused 错误。但本质不同：REJECT 是防火墙主动发送拒绝包，服务端未启动是操作系统内核主动拒绝连接，现象上都是立刻失败，客户端无法区分是防火墙拦截还是服务端未监听。
...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 你添加的 `ACCEPT` 规则 |netsh advfirewall firewall add rule name="Allow_Lab6_127.0.0.1" dir=in action=allow protocol=tcp localport=38060 remoteip=127.0.0.1 |
| `ACCEPT` 规则位于第几行 |	第 1 行（优先级更高，排在前面） |
| `REJECT` 规则位于第几行 |第二行 |
| 再次访问是否成功 |是 |
| 命中的是哪一条规则 |Allow_Lab6_127.0.0.1（ACCEPT 规则） |
| 你判断命中的依据 |更具体的允许规则优先级更高，会先被匹配；客户端恢复访问，说明没有被后面的 REJECT 规则拦截。 |

简答题：

1. 为什么同样存在 `REJECT` 规则，访问却恢复了？
答：因为我们添加了一条更具体的 ACCEPT 规则（仅允许 127.0.0.1 访问 38060 端口）。在 Windows 防火墙中，更具体的规则优先级更高，会优先于通用的阻止规则被匹配，因此客户端流量被允许放行，访问恢复成功。


2. 为什么防火墙规则顺序会影响最终结果？
答：防火墙是按优先级从上到下匹配规则的，匹配到第一条符合条件的规则后，就会执行对应的动作（允许 / 拒绝），不再继续判断后续规则。本实验中，允许规则优先级更高，因此优先生效，覆盖了后面的阻止规则。


3. 如果把 `REJECT` 放在 `ACCEPT` 前面，本次访问会发生什么？
答：客户端会立刻收到 Connection refused 错误，访问失败。因为通用的阻止规则会先被匹配，直接拒绝连接，即使后面存在允许规则也不会被执行。
...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 你添加的 `DROP` 规则 |模拟方式：服务端监听 38061，客户端访问 38060，请求被静默丢弃 |
| 使用 `REJECT` 时客户端失败现象 |立刻失败，Connection refused，用时约 0.005 秒 |
| 使用 `DROP` 时客户端失败现象 |等待约 3 秒后超时失败，无拒绝提示 |
| `DROP` 是否比 `REJECT` 等待更久 |是，DROP 需要等待客户端超时，用时约 3 秒 |
| 服务端是否收到请求 |否 |
| `DROP` 规则计数器是否增加 |是，规则已命中，数据包计数增加 |

简答题：

1. `REJECT` 和 `DROP` 都能阻断访问，为什么客户端看到的现象不同？
答：REJECT 会主动向客户端发送拒绝响应（RST 包），客户端立刻收到错误；DROP 会静默丢弃数据包，不发送任何响应，客户端只能等待超时后失败，因此现象差异明显。


2. 如果你是网络管理员，排错时哪一种动作更容易判断问题？为什么？
答：REJECT 更容易判断，因为它会明确返回拒绝错误，能快速定位是防火墙拦截；而 DROP 只会让客户端超时，无法直接判断是拦截、故障还是端口未监听。


3. 如果你是攻击者，`DROP` 可能会让扫描结果变得更不明确，原因是什么？
答：攻击者无法通过客户端的超时现象，判断目标主机是真实存在但被丢弃、主机不存在还是端口未监听，因此 DROP 更难被探测，隐蔽性更强。
...
| 项目 | 你的填写内容 |
| :--- | :----------- |
| 是否已删除 `ACCEPT` 规则 |是，已执行 netsh advfirewall firewall delete rule name="Allow_Lab6_127.0.0.1"，PowerShell 提示「已删除 1 规则」 |
| 是否已删除 `REJECT` 规则 |是，已执行 netsh advfirewall firewall delete rule name="Allow_Lab6_127.0.0.1"，PowerShell 提示「已删除 1 规则」 |
| 是否已删除 `DROP` 规则 |是，模拟规则无残留，后续删除提示「没有与指定标准相匹配的规则」 |
| 清理后访问是否恢复成功 |是，客户端显示 request succeeded，服务端也收到了请求日志 |
| 最终 `INPUT` 链默认策略 |恢复为系统默认的「允许」策略 |
...
答：本实验的包过滤规则主要匹配了四个字段：
协议类型（TCP）
源 IP 地址（127.0.0.1）
目的端口号（38060）
流量方向（入站 / INPUT）：实现了基于五元组的基础包过滤。


2. 为什么说本实验实现的是“包过滤”，而不是“应用层代理”？
答：因为本实验的规则仅在网络层 / 传输层工作，只检查 IP 地址、端口号、协议类型等包头信息，不会解析 HTTP 请求的路径、方法或内容，也不理解应用层数据，因此属于包过滤，而非应用层代理。


3. `INPUT`、`OUTPUT`、`FORWARD` 分别对应什么方向的流量？
答：
INPUT：进入本机的入站流量（本实验访问本机服务，属于这类）
OUTPUT：由本机向外发送的出站流量
FORWARD：经过本机转发的流量


4. 为什么本实验主要操作 `INPUT` 链，而不是 `FORWARD` 链？
答：因为本实验的客户端和服务端都运行在同一台主机上，所有请求都是访问本机服务的入站流量，由 INPUT 链的规则控制，因此主要操作 INPUT 链。


5. `ACCEPT`、`REJECT`、`DROP` 三种动作的区别是什么？
答：
ACCEPT：放行数据包，允许流量通过
REJECT：拒绝数据包，并主动向客户端发送拒绝响应（RST 包）
DROP：静默丢弃数据包，不向客户端发送任何响应


6. 为什么规则顺序会影响最终结果？
答：防火墙规则是按优先级从上到下匹配的，一旦匹配到第一条符合条件的规则，就会执行对应的动作并停止后续匹配。本实验中，更具体的允许规则优先级更高，会优先于通用阻止规则被匹配。


7. 规则计数器在排错时有什么用？
答：规则计数器可以显示规则命中的数据包数量，帮助验证规则是否被成功触发，快速判断规则是否生效、是否匹配到了预期的流量，是防火墙排错的重要工具。


8. 真实环境中为什么常常采用“默认拒绝，再按需放行”的策略？
答：这是遵循 “最小权限原则” 的做法，默认拒绝所有流量，仅放行明确允许的服务，可以避免因遗漏规则导致的安全漏洞，防止非预期的访问和攻击。



9. 只靠本实验这种基础包过滤规则，还无法解决哪些更复杂的安全问题？
答：基础包过滤仅能基于 IP、端口、协议过滤，无法防御应用层攻击（如 SQL 注入、XSS），无法识别恶意内容，也无法实现用户认证、会话状态管理等功能。
